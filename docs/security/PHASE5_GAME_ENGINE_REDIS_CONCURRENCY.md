...
**Sources:** `apps/engines/bingo/` (Railway workers + optional HTTP API), PostgreSQL RPCs in `supabase/schema.sql` and `winway/sql/migrations/`, player paths in `services/`, `app/api/`.  
...
Configured in `apps/engines/bingo/src/config/env.ts`; roles via `GAME_ENGINE_ROLES` (comma-separated: `scheduler`, `draw-processor`, `room-loop`, …).
...
| Path | Topic |
|------|--------|
| `apps/engines/bingo/src/index.ts` | Worker startup, Redis connect |
| `apps/engines/bingo/src/workers/draw-processor/pickCoordinator.ts` | Pick lock, dispatch |
| `apps/engines/bingo/src/workers/draw-processor/adaptivePollScheduler.ts` | Poll backoff |
| `apps/engines/bingo/src/workers/room-loop/runDrawCycle.ts` | Authoritative draw cycle |
| `apps/engines/bingo/src/domain/draw/processEngineDrawJob.ts` | Finalize + settle |
| `apps/engines/bingo/src/domain/draw/filterActorOwnedDrawJobs.ts` | Actor vs processor split |
| `apps/engines/bingo/src/redis/leaderLock.ts` | Degraded multi-replica behavior |
| `apps/engines/bingo/src/http/commands.ts` | Client command surface |
| `apps/engines/bingo/src/core/rng.ts` | Provably-fair pick |
