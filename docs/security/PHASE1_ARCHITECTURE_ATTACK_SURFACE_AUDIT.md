...
| **Game engine** | Node package `apps/engines/bingo/` (Railway per code comments) | Background workers + optional HTTP command API |
...
| **Direct PostgreSQL** | `lib/pg.ts` (`DATABASE_URL`), `apps/engines/bingo/src/db/pg.ts` | PG-first snapshots (gameroom, live-room, card pool) bypassing RLS |

### Game engine workers (in-process, not HTTP)

From `apps/engines/bingo/src/index.ts` and `apps/engines/bingo/src/config/env.ts`:
...
3. **Browser ↔ Game Engine** — JWT verified in `apps/engines/bingo/src/http/auth.ts`; CORS from `GAME_ENGINE_CORS_ORIGINS` (default `*`).
...
### 2. Game Engine HTTP (`apps/engines/bingo/src/http/server.ts`)
...
- **File:** `apps/engines/bingo/src/http/cors.ts`
...
8. **Redis key layout** (`apps/engines/bingo/src/redis/keysV2.ts`) — Impact if Redis compromised.
