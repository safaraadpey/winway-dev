...
| Repo runtime | **A — hybrid only:** `game-engine/src/workers/room-scheduler/index.ts` → `supabase.rpc("fn_heartbeat_tick")` when `GAME_RUNTIME` is not `engine`/`legacy_db` idle path |
...
- Repo: `game-engine/`, `app/`, `scripts/`, `sql/`, `docs/`, `src/types/supabase.ts`
