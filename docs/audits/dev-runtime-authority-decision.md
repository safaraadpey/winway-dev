...
With `SCHEDULER_ENABLED=true` and the roles above, `game-engine` starts these workers (`game-engine/src/index.ts` + role handlers). Behavior under `GAME_RUNTIME=engine`:
