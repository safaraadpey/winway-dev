...
| `SUPABASE_URL` | String | required (engine) | `apps/engines/bingo/src/config/env.ts:136` via `requireEnv` | Engine Supabase client | Engine throws on boot | Engine-only name (not `NEXT_PUBLIC_*`) | Core for engine | Critical | **C** |
| `DATABASE_URL` | Secret URI | unset → PG pool null | Next: `lib/pg.ts:9-14`; Engine: `apps/engines/bingo/src/db/pg.ts:4`; logged presence in `apps/engines/bingo/src/index.ts:122` | Direct PG snapshots / engine PG | Supabase SDK fallback | Vercel + Railway | **Both read paths live** | High for correctness | **C** — keep (PostgREST incident history) |
...
| `GAME_RUNTIME` | Enum | parse → `legacy_db` | `apps/engines/bingo/src/config/env.ts:124-126,142`; `runtime.ts:13-25`; all workers | See Runtime Selectors | Cron-owned when `legacy_db` | Roles + SCHEDULER + cron mutex | **All 3 modes executable** | **Critical** misconfig | **C/D** — architecture decision |
...
| `GAME_ENGINE_CORS_ORIGINS` | String CSV | `*` (cors.ts) | `apps/engines/bingo/src/http/cors.ts:8` | Allowed browser origins | Default `*` insecure for prod | Browser ENGINE_PATH | **نیازمند بررسی** prod | High security | **C** |
...
**Parse:** `apps/engines/bingo/src/config/env.ts:124-126` — unknown/missing → **`legacy_db`**.  
**Helpers:** `apps/engines/bingo/src/runtime.ts:13-25`.
...
→ loadConfig() (apps/engines/bingo/src/config/env.ts)
...
|------|-------------|------------|------------|
| Root `@upstash/redis` dependency | Only imported in `apps/engines/bingo/src/redis/client.ts`; Next app does not import | High | Do not delete package without checking workspace install model — **B** |
...
| `apps/engines/bingo/.env.example` | Defaults `GAME_RUNTIME=engine`, `SCHEDULER_ENABLED=false` |
| `apps/engines/bingo/.env.develop.local.example` | `GAME_RUNTIME=legacy_db`, limited roles, scheduler off |
...
| Engine config | `apps/engines/bingo/src/config/env.ts`, `apps/engines/bingo/src/runtime.ts`, `apps/engines/bingo/src/index.ts` |
| Workers | `apps/engines/bingo/src/workers/**` |
| PG access | `lib/pg.ts`, `apps/engines/bingo/src/db/pg.ts` |
| Env examples | `.env.local.example`, `.env.develop.local.example`, `apps/engines/bingo/.env.example`, `apps/engines/bingo/.env.develop.local.example` |
