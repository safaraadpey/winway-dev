...
function fromApi(t: any): TierRow {
  return {
    key: String(t.id),
    id: String(t.id),
...
export default function CryptoPaymentManager() {
  const [activeTab, setActiveTab] = useState<"tiers" | "xpub">("tiers");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows((data.tiers || []).map(fromApi));
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-[#0E0E0F] p-4">
      <div className="mx-auto max-w-4xl">
        <h1 className="mb-2 text-center text-xl font-bold text-white">
          مدیریت پرداخت کریپتو
        </h1>
        <p className="mb-5 text-center text-sm leading-6 text-gray-400">
          ضرایب پلکانی فاکتور و کلیدهای XPUB پایه برای تولید آدرس واریز کاربران.
        </p>

        <div className="mb-5 grid grid-cols-2 gap-2 rounded-xl bg-[#151515] p-1">
          <button
            type="button"
          ...
          >
            کلیدهای XPUB
            {!canManageXpub && !xpubLoading ? " (مدیر کل)" : ""}
          </button>
        </div>
        ...
        {activeTab === "tiers" &&
          ...
          <>
            <div className="mb-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  setRows((prev) => [
                    ...prev,
                    newRow({ sortOrder: String((prev.length + 1) * 10) }),
                  ])
                }
            ...
            <p className="mt-3 text-xs leading-5 text-gray-500">
              ...
              جبران کارمزد شبکه. بونوس٪ روی کارت فاکتور به کاربر نمایش داده می‌شود.
            </p>
          </>
        ))}
      </div>
