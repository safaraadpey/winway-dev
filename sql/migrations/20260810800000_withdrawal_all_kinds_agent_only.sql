...
-- All withdrawal kinds (rial + crypto): only the assigned upstream agent may review.
-- Admin/super must not list, process, approve, or reject.
...
as $$
begin
  -- Same gate as rial: assigned agent only (never admin/super).
  return public.fn_withdrawal_actor_can_review(p_actor_id, p_player_id);
end;
$$;

-- Keep legacy 1-arg overload fail-closed (no longer grants admin access).
create or replace function public.fn_withdrawal_actor_can_review_crypto(
  p_actor_id uuid
)
returns boolean
...
drop function if exists public.fn_withdrawal_request_approve_crypto(uuid, uuid);
...
begin
  ...
  if v_req.agent_id is null then
  ...
  -- Credit assigned agent wallet (not admin/super), matching rial approve.
