...
alter type public.withdrawal_request_status add value if not exists 'processing';

-- Player cancels while still pending (releases hold).
create or replace function public.fn_withdrawal_request_cancel(
  p_request_id uuid,
  p_player_id uuid
)
returns table(
...
begin
  if p_request_id is null or p_player_id is null then
    raise exception 'request_id and player_id required';
  end if;
  ...
  if v_req.player_id <> p_player_id then
  ...
  return query
    select v_req.id, 'cancelled'::public.withdrawal_request_status, false;
end;
$$;

-- Agent/admin marks request as in-progress (player can no longer cancel).
...
-- Rial approve: allow from pending or processing
...
-- Crypto approve: allow from pending or processing
...
-- Reject: allow from pending or processing
...
revoke all on function public.fn_withdrawal_request_cancel(uuid, uuid) from public;
revoke all on function public.fn_withdrawal_request_mark_processing(uuid, uuid) from public;
grant execute on function public.fn_withdrawal_request_cancel(uuid, uuid) to service_role;
grant execute on function public.fn_withdrawal_request_mark_processing(uuid, uuid) to service_role;
