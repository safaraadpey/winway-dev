-- Migration: fix OUT parameter ambiguity inside game_core.fn_join_or_create_room_core
-- تاریخ: 2025-11-28
--
-- هدف:
-- 1) معرفی متغیر محلی (v_starts_at) به جای استفاده‌ی مستقیم از پارامتر خروجی starts_at
--    تا ارجاع به ستون‌های rooms.starts_at / select های داخلی مبهم نباشد.
-- 2) ایجاد دوباره‌ی فانکشن core با همین منطق قبلی اما بدون استفاده از OUT params
--    تا مرحله‌ی نهایی (room_id, starts_at, ticket_ids).
...
DECLARE
  ...
  v_starts_at      timestamptz; -- خروجی نهایی starts_at
BEGIN
