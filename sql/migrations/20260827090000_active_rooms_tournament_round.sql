-- Active-game chips: tournament rooms show round_no (تور / round).
...
BEGIN
  ...
  SELECT
    ...
    (
      ...
      WHERE trr.room_id = pr.room_id
      ORDER BY trr.round_no DESC
      LIMIT 1
    ) AS round_no
...
COMMENT ON FUNCTION public.fn_my_active_rooms(uuid) IS
  'Player active rooms snapshot. template_table_index is 1-based among live rooms of the same template. round_no is the tournament round for tournament rooms.';
