-- Keep engine secondary tables (card_definition_masks / card_number_index) in sync
-- with card_numbers whenever a staged card pool finishes building.
--
-- Root cause: new pools wrote card_numbers via trg_sync_card_numbers without
-- bit_position / mask / index rows, so engine win detection skipped those cards.

BEGIN;

-- Ensure secondary tables cover all existing card_numbers (idempotent).
DO $$
DECLARE
  v_bitmask record;
BEGIN
  ...
  RAISE NOTICE
    '[CardPool] migration bitmask backfill: cards=%, index=%, masks=%',
...
ALTER FUNCTION game_core.fn_generate_card_pool_step(integer) OWNER TO postgres;
