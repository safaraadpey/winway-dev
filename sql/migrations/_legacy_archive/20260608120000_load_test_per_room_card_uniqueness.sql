-- Fix load-test seed: card uniqueness per room (matches fn_system_join), not global.
-- Same pool_card may be reused across concurrent rooms.

BEGIN;

DROP FUNCTION IF EXISTS load_test._available_pool_cards(uuid, int);
...
BEGIN
  ...
  -- Per-room capacity: same pool cards may be reused across rooms.
