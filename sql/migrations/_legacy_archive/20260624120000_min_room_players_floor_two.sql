-- Enforce minimum 2 players to start a room (template, join, and promotion paths).
BEGIN;

UPDATE public.room_templates
   SET min_players = 2
 WHERE min_players IS NULL OR min_players < 2;

UPDATE public.rooms
   SET min_players = 2
 WHERE (min_players IS NULL OR min_players < 2)
   AND status IN ('waiting'::public.room_status, 'playing'::public.room_status);

ALTER TABLE public.room_templates
  ALTER COLUMN min_players SET DEFAULT 2;

ALTER TABLE public.room_templates
  DROP CONSTRAINT IF EXISTS room_templates_min_players_check;

ALTER TABLE public.room_templates
  ADD CONSTRAINT room_templates_min_players_check
  CHECK (min_players >= 2);
