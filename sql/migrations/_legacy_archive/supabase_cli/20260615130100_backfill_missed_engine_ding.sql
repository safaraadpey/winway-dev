-- One-time backfill for draws processed while rpc_finalize called ding before processed_at.
-- Reconstructs credits from marks (same basis as engine) for rows where
-- processed_at IS NOT NULL AND ding_aggregated_at IS NULL.
  ...
  LOOP
    ...
    IF jsonb_array_length(v_credits) = 0 THEN
      UPDATE public.draws
       ...
       WHERE room_id = v_draw.room_id
         AND number = v_draw.draw_number
