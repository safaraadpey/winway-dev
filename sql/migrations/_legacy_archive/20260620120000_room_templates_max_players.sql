-- Add max_players to room_templates; copy to rooms on join; block join when full;
-- promote waiting rooms to playing immediately when capacity is reached.
BEGIN;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'room_templates_max_players_check'
      AND conrelid = 'public.room_templates'::regclass
  ) THEN
    ALTER TABLE public.room_templates
      ADD CONSTRAINT room_templates_max_players_check
...
BEGIN
  ...
  FOR r IN
    ...
    WHERE rm.status = 'waiting'
      AND rm.max_players IS NOT NULL
      AND (
        SELECT COUNT(DISTINCT t.player_user_id)
        FROM public.tickets t
