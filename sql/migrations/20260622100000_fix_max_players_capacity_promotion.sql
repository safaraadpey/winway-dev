-- Fix max_players capacity: sync room snapshot, align system join, template fallback
-- for promotion, and immediate promote when capacity is reached on join.
BEGIN;

-- Backfill waiting rooms created before max_players was copied (e.g. dev-player join).
UPDATE public.rooms r
   SET max_players = rt.max_players,
       meta = COALESCE(r.meta, '{}'::jsonb) || jsonb_build_object('max_players', rt.max_players),
       updated_at = now()
  FROM public.room_templates rt
 WHERE r.room_template_id = rt.id
   AND r.status = 'waiting'::public.room_status
   AND r.max_players IS NULL
   AND rt.max_players IS NOT NULL;
...
ALTER FUNCTION game_core.fn_try_promote_room_at_max_capacity(uuid) OWNER TO postgres;
