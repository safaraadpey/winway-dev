-- Attributed admin_amount on agent/super daily rows (admin panel breakdown snapshot read).
-- Operator downline game rollup unchanged from 20260903200000.
...
BEGIN
  ...
  agent_downline_game AS (
    SELECT
      pa.agent_id AS user_id,
      COALESCE(SUM(pc.cards_count), 0)::bigint AS cards_count,
      COALESCE(SUM(pc.cards_amount), 0) AS cards_amount,
      COALESCE(SUM(pc.games_played), 0)::bigint AS games_played,
      COALESCE(SUM(pnw.player_winnings), 0) AS player_winnings
    FROM public.player_affiliation pa
    LEFT JOIN player_cards pc ON pc.user_id = pa.user_id
    LEFT JOIN player_normal_wins pnw ON pnw.user_id = pa.user_id
    WHERE pa.agent_id IS NOT NULL
    GROUP BY pa.agent_id
  ),
  super_downline_game AS (
    SELECT
      pa.super_id AS user_id,
      COALESCE(SUM(pc.cards_count), 0)::bigint AS cards_count,
      COALESCE(SUM(pc.cards_amount), 0) AS cards_amount,
      COALESCE(SUM(pc.games_played), 0)::bigint AS games_played,
      COALESCE(SUM(pnw.player_winnings), 0) AS player_winnings
  ...
  operator_admin_attributed AS (
    SELECT
      e.user_id,
      e.role,
      COALESCE(SUM(e.earned), 0) AS admin_amount
    FROM (
      SELECT
  ...
  agent_rows AS (
    SELECT
      u.id AS user_id,
      'agent'::text AS role,
      COALESCE(adg.cards_count, 0) AS cards_count,
      COALESCE(adg.cards_amount, 0) AS cards_amount,
      COALESCE(adg.games_played, 0) AS games_played,
      COALESCE(adg.player_winnings, 0) AS player_winnings,
      ...
      0::numeric AS super_amount,
      COALESCE(oaa.admin_amount, 0) AS admin_amount,
      0::numeric AS direct_player_amount,
    ...
    FROM public.users u
    LEFT JOIN agent_downline_game adg ON adg.user_id = u.id
    LEFT JOIN agent_ticket at ON at.user_id = u.id
    LEFT JOIN agent_tournament atr ON atr.user_id = u.id
    LEFT JOIN operator_admin_attributed oaa ON oaa.user_id = u.id AND oaa.role = 'agent'::text
    ...
    WHERE u.role = 'agent'::public.user_role
      AND (
        ...
        OR COALESCE(adg.cards_amount, 0) > 0
        OR COALESCE(adg.player_winnings, 0) > 0
        OR COALESCE(oaa.admin_amount, 0) > 0
      )
  ),
  super_ticket AS (
    SELECT
  ...
  super_rows AS (
    SELECT
      u.id AS user_id,
      'super'::text AS role,
      COALESCE(sdg.cards_count, 0) AS cards_count,
      COALESCE(sdg.cards_amount, 0) AS cards_amount,
      COALESCE(sdg.games_played, 0) AS games_played,
      COALESCE(sdg.player_winnings, 0) AS player_winnings,
      0::numeric AS tournament_winnings,
      0::bigint AS line_wins_count,
      0::bigint AS full_wins_count,
      0::numeric AS agent_amount,
      ...
      0::numeric AS approved_withdrawals
    FROM public.users u
    LEFT JOIN super_downline_game sdg ON sdg.user_id = u.id
    LEFT JOIN super_ticket st ON st.user_id = u.id
    LEFT JOIN super_tournament str ON str.user_id = u.id
    LEFT JOIN operator_admin_attributed oaa ON oaa.user_id = u.id AND oaa.role = 'super'::text
    LEFT JOIN operator_manual_panel omp ON omp.user_id = u.id
    LEFT JOIN operator_transfer ot ON ot.user_id = u.id
    WHERE u.role = 'super'::public.user_role
      AND (
        ...
        OR COALESCE(omp.panel_deposits, 0) + COALESCE(ot.panel_deposits, 0) > 0
        OR COALESCE(omp.panel_withdrawals, 0) + COALESCE(ot.panel_withdrawals, 0) > 0
        OR COALESCE(sdg.cards_amount, 0) > 0
        OR COALESCE(sdg.player_winnings, 0) > 0
        OR COALESCE(oaa.admin_amount, 0) > 0
      )
  ),
  admin_fee_ticket AS (
