-- Lifetime performance snapshot (overall = SUM of all closed daily rows through last snapshot_date).
-- Rebuilt idempotently inside fn_performance_snapshot_run after each daily close.

BEGIN;

CREATE TABLE IF NOT EXISTS public.performance_lifetime_stats (
  user_id uuid NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
  role text NOT NULL CHECK (role IN ('player', 'agent', 'super', 'admin')),
  currency text NOT NULL DEFAULT 'IRR',
  through_snapshot_date date NOT NULL,
...
COMMENT ON TABLE public.performance_lifetime_stats IS
  'Overall base metrics per user/role through through_snapshot_date (last closed 08:00–08:00 Tehran day). Rebuilt from SUM(daily) after each snapshot run.';

CREATE INDEX IF NOT EXISTS idx_performance_lifetime_stats_role
  ON public.performance_lifetime_stats (role);
...
DROP FUNCTION IF EXISTS public.fn_performance_snapshot_run(date);

-- Recreate snapshot run with lifetime rebuild (body matches 20260902140000 + rebuild step).
...
ALTER TABLE public.performance_lifetime_stats ENABLE ROW LEVEL SECURITY;

CREATE POLICY performance_lifetime_stats_admin_read
  ON public.performance_lifetime_stats
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1
...
CREATE POLICY performance_lifetime_stats_self_read
  ON public.performance_lifetime_stats
  FOR SELECT
  USING (user_id = auth.uid());

REVOKE ALL ON TABLE public.performance_lifetime_stats FROM PUBLIC, anon;
GRANT SELECT ON TABLE public.performance_lifetime_stats TO authenticated, service_role;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.performance_lifetime_stats TO service_role;
