-- Separate table size settings for round 1 vs later rounds.

BEGIN;

ALTER TABLE public.tournaments
  ADD COLUMN IF NOT EXISTS later_round_table_size_mode public.tournament_table_size_mode,
  ADD COLUMN IF NOT EXISTS later_round_table_size_fixed integer,
  ADD COLUMN IF NOT EXISTS later_round_table_size_min integer,
  ADD COLUMN IF NOT EXISTS later_round_table_size_max integer;

ALTER TABLE public.tournaments
  DROP CONSTRAINT IF EXISTS tournaments_later_round_table_size_fixed_check;

ALTER TABLE public.tournaments
  ADD CONSTRAINT tournaments_later_round_table_size_fixed_check CHECK (
    (later_round_table_size_mode IS NULL)
    OR (later_round_table_size_mode <> 'fixed'::public.tournament_table_size_mode)
    OR (
      (later_round_table_size_fixed IS NOT NULL)
      AND (later_round_table_size_fixed >= 2)
    )
  );

ALTER TABLE public.tournaments
  DROP CONSTRAINT IF EXISTS tournaments_later_round_table_size_range_check;

ALTER TABLE public.tournaments
  ADD CONSTRAINT tournaments_later_round_table_size_range_check CHECK (
    (later_round_table_size_mode IS NULL)
    OR (later_round_table_size_mode <> 'range'::public.tournament_table_size_mode)
    OR (
      (later_round_table_size_min IS NOT NULL)
      AND (later_round_table_size_max IS NOT NULL)
      AND (later_round_table_size_min >= 2)
      AND (later_round_table_size_max >= later_round_table_size_min)
    )
  );
