...
describe("evaluateDrawInRam", () => {
  it("evaluates in memory without recordDrawProcessed", () => {
    const state = makeState();
    const result = evaluateDrawInRam(state, 7, registry);
    assert.equal(result.persistence.marks.length, 1);
    assert.equal(state.drawsProcessed, 0);
