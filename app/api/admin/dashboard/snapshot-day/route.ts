...
import { loadAdminDashboardDaySnapshot } from "@/lib/dashboard/loadAdminDashboardSnapshot";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** GET /api/admin/dashboard/snapshot-day — live day summary (Tehran 08:00 → now). */
export async function GET(request: NextRequest) {
  try {
    const { userId, supabase } = await resolveAdminDashboardRequestAuth(request);
    await assertAdminPanelAccess(userId);

    const data = await loadAdminDashboardDaySnapshot(supabase);
    ...
    console.error("[DashboardSnapshot] snapshot-day unexpected error:", err);
    return NextResponse.json(
