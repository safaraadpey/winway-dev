...
export async function GET(request: Request) {
  try {
    ...
    if (templatesError) {
      console.error("GET /api/player/lobby-room-groups templates error:", templatesError);
      return NextResponse.json(
    ...
    if (roomsError) {
      console.error("GET /api/player/lobby-room-groups rooms error:", roomsError);
      return NextResponse.json(
    ...
    if (ticketsError) {
      console.error("GET /api/player/lobby-room-groups tickets error:", ticketsError);
      return NextResponse.json(
    ...
    // Output: only template groups + any fallback groups created by active rooms.
    const payload: LobbyRoomGroup[] = Array.from(groups.values()).sort(
      (a, b) => a.price - b.price
    );

    return NextResponse.json({ groups: payload });
  } catch (err: any) {
    console.error("GET /api/player/lobby-room-groups error:", err);
    return NextResponse.json(
      { error: "internal_error", message: err?.message || "Failed to load lobby groups." },
      { status: 500 }
    );
  }
}
