...
import { cancelTournamentRegistrationCampaign } from "@/services/dev-panel/tournament-register";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!campaignId) {
      return NextResponse.json(
        { ok: false, error: "validation_error", message: "campaignId is required" },
        { status: 400 }
      );
    }

    const data = await cancelTournamentRegistrationCampaign({ campaignId });
    return NextResponse.json({ ok: true, data });
  } catch (err) {
    console.error("[DevRegister] POST cancel error:", err);
