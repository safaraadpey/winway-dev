...
import { loadOperatorDashboardDaySnapshot } from "@/lib/dashboard/loadOperatorDashboardSnapshot";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/** GET /api/agent/dashboard/snapshot-day — live day summary (Tehran 08:00 → now). */
export async function GET(request: NextRequest) {
  try {
    const { supabase } = await resolveAdminDashboardRequestAuth(request);
    const data = await loadOperatorDashboardDaySnapshot(supabase);
    ...
    console.error("[DashboardSnapshot] operator snapshot-day unexpected error:", err);
    return NextResponse.json(
