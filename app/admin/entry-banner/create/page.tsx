...
export default function CreateEntryBannerPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const result = await createEntryBanner(formData);

      if (result.success) {
        toast.success("بنر با موفقیت ایجاد شد");
        router.push("/admin/entry-banner");
      } else {
        toast.error(result.error || "خطا در ایجاد بنر");
      }
    } catch (error: any) {
      console.error("Error creating banner:", error);
      toast.error(error.message || "خطا در ایجاد بنر");
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0E0E0F] text-white p-4">
      <div className="max-w-md mx-auto">
        <h1 className="text-2xl font-bold mb-4">ایجاد بنر ورودی</h1>
          ...
          {formData.contentType === "text" ? (
            ...
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl bg-[#1f2933] text-white px-4 py-3 outline-none border border-transparent focus:border-teal-500"
                required={formData.contentType === "image"}
              />
              {formData.imageFile && (
                <div className="mt-2 text-sm text-gray-400">
                  فایل انتخاب شده: {formData.imageFile.name} ({(formData.imageFile.size / 1024).toFixed(2)} KB)
                </div>
              )}
            </div>
          )}
