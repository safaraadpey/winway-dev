...
$engineDevelop = Join-Path $Root "game-engine\.env.develop.local"
$engineEnv = Join-Path $Root "game-engine\.env"
if (Test-Path $engineDevelop) {
  Copy-Item $engineDevelop $engineEnv -Force
  Write-Host "Active: game-engine/.env -> develop"
}

Write-Host "Restart npm run dev and game-engine to pick up changes."
